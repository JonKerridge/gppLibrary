...
class JUtest33Log {
    ...
    public void test() {
        ...
        def logVis = new LoggingVisualiser( logInput: logChan.in(),
                collectors: workers,
                logFileName: "D:\\IJGradle\\gppLibrary\\src\\test\\groovy\\gppJunitTests\\LogFiles/33")
...
//        println "Result is ${compDetails.toString()}"
        ...
        def gopC = new GroupOfPipelineCollects(groups: workers,
        stages: stages,
        inputAny: chan2.in(),
        ...
        PAR testParallel = new PAR([logVis, emitter, ofa, gopC])
        testParallel.run()
